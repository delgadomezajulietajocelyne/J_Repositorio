...
public class ArbolBinarioOrdenado {
    class Nodo {
      ...
      public boolean existe(Linea linea_ens,FileWriter archInst, int numeroLinea, FileWriter archErr, ContadorDeLocalidades contlocalidades,ArbolEtiquetas arboletq,FileWriter archTABSIM, String nombrearch) {
        Nodo reco=raiz;
        try{
        	while (reco!=null) {
        		if (linea_ens.codop.equalsIgnoreCase(reco.codop)) {
                	if (reco.existeoperando == true && !linea_ens.oper.equals("NULL")) {
                		...
                		oper.VerificarInicioDeOperador(linea_ens.oper,reco.modosdir,archErr,numeroLinea,linea_ens, archInst,reco.sumabytes,contlocalidades,arboletq,archTABSIM,nombrearch);
                		/*for(int i = 0;i<reco.modosdir.size();i++){
      ...
      public boolean escribirCodMaq(String codigoOperacion,String mododir,ArbolBinarioOrdenado abo, FileWriter arch,String oper, ArbolEtiquetas arboletq,String conloc, long point, long point2,RandomAccessFile archErr,RandomAccessFile archI,String numeroLinea, Linea linea_ens) {
        Nodo reco=raiz;
        String codmaqenhex = "";
        int minusc = 0;
        int sumb = 0;
